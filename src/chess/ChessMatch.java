...
public class ChessMatch {
	...
	public ChessPiece replacePromotedPiece(String type) {
		if (promoted == null) {
			throw new IllegalStateException("Não tem peças para serem promovidas");
		}
	...
	private ChessPiece newPiece(String type, Color color) {
		if (type.equals("Bispo")) return new Bispo(board, color);
		if (type.equals("Cavalo")) return new Cavalo(board, color);
		if (type.equals("Rainha")) return new Rainha(board, color);
	...
	private void validateTargetPosition(Position source, Position target) {
		if (!board.piece(source).possibleMove(target)) {
			throw new ChessException("A  peça escolhida não pode se mover pra posição alvo");
		}
	}
	...
	private ChessPiece king(Color color) {
		...
		throw new IllegalStateException("Não existe rei " + color + " no campo");
	}
